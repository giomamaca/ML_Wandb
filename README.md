...
sanity checks-ს. ისინი მოკლე დროში მეოუბნებიან კოდი გამართულია თუ არა, და ამით ავიცილე bug-ები თავიდან
...
დაახლოებით თანაბრად უნდა ანაწილებდეს, ამიტომ cross-entropy loss საწყისად მიახლოებით ln(7) =
1.946 უნდა იყოს. თუ ეს რიცხვი სრულიად სხვაა, ესე იგი სადღაც bug-ია — არასწორი
ინიციალიზაცია, არასწორი კლასების რაოდენობა ან ლეიბლების არევა.
...
მესამეა გრადიენტების შემოწმება: ერთი backward-ის შემდეგ ყველა ფენას უნდა
ჰქონდეს სასრული, არანულოვანი გრადიენტი. ნულოვანი გრადიენტი მიუთითებს მონაცემთა უცვლელობაზე,
...
├── README.md
├── requirements.txt
├── experiments.ipynb       # მთავარი: setup + ყველა ექსპერიმენტი + ჰიპერპარამეტრები (GPU)
...
└── src/
    ...
    ├── train.py             # სატრენინგო ციკლი + W&B logging
...
ჩემი მოლოდინი იყო underfitting, და ზუსტად ასეც მოხდა: ვალიდაციის accuracy მიახლოებით 0.41-ზე
გაჩერდა და train-თან ერთად, თითქმის პარალელურად მიდიოდა (gap დაახლოებით 0.05, ძალიან პატარა).
...
აქ ერთი დეტალია, რომელზეც განსაკუთრებით ღირს მსჯელობა: ვალიდაციის accuracy მიახლოებით 0.55-ზე
ჩერდება, მაგრამ ვალიდაციის loss 1.25-დან 4-მდე იზრდება. ერთი შეხედვით ეს
...
შედეგი მკაფიოა: ვალიდაციის საუკეთესო accuracy მიახლოებით 0.65-მდე ავიდა საუკეთესო ყველა
„სუფთა" არქიტექტურიდან. თუმცა სრულად overfitting არ აღმოფხვრილა: train 0.9-ს ცოტათი გასცდა,
gap 0.27, და val loss მინიმუმს მე-8 epoch-ზე აღწევს (1.045), შემდეგ კი ისევ იზრდება (1.44-მდე).
ანუ რეგულარიზაციამ პრობლემა მნიშვნელოვნად შეამცირა, მაგრამ მოდელს ჯერ კიდევ აქვს ჭარბი
ტევადობა. სწორედ ეს მიჩვენებს, რომ შემდეგი ნაბიჯი data augmentation-ია.
...
**ResNet**: val **~0.60**, მაგრამ train **0.98** და gap **0.39**,
...
**GoogLeNet / Inception** (პარალელური 1×1/3×3/5×5 ფილტრები): val დაახლოებით **0.60**, train 0.85,
gap მიახლოებით 0.25.
